...
def process_images_in_directory(directory_path):
    for filename in os.listdir(directory_path):
        if filename.endswith(".jpg") or filename.endswith(".png"):
            ...
            save_graph(G, os.path.join(directory_path, graph_filename))
            print(f"Processed {filename} and saved graph as {graph_filename}")

            # Optional: Visualize the graph (comment out if not needed)
            pos = nx.get_node_attributes(G, 'pos')
            nx.draw(G, pos, with_labels=True, node_size=20, font_size=10)
            plt.show()

# Example Usage
process_images_in_directory('Himanshu_CV/dataset_himasnhu')
